	...
	function createDivelogs($post_data) {
		...
		for ($i = 0; $i < $post_data['num_of_divelogs']; $i++) {
			...
			try {
				...
				$new_divelog->title = elgg_view('object/divelog/text', array('entity'=>$new_divelog, 'mode'=>'title'));
				$new_divelog->description = elgg_view('object/divelog/text', array('entity'=>$new_divelog, 'mode'=>'description'));
