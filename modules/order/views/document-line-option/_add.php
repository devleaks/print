...
$order_line_option = new DocumentLineOption();
$order_line_option->document_line_id = $order_line->id;
$count = 0;
$attributes = [];
foreach(ItemOption::find()->where(['item_id'=>831]))->orderBy('position')->each() as $io) {
	...
	$attributes['dummy[]'] = [
	    ...
	    'form' => $form,
	    'columns' => 5,
		'value' =>	Html::activeHiddenInput($order_line_option.'[]', 'document_line_id').
