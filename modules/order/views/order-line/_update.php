...
if(!isset($model->image_add)) $model->image_add = OrderLine::IMAGE_ADD;
	...
	<?= Form::widget([
			...
			'attributes' => [       // 2 column layout
				'item_id' => [
					...
					// <input type="text" id="orderline-item_id" class="form-control kv-hide input-md" name="OrderLine[item_id]">
							 . Html::hiddenInput('item_id', $model->item_id, ['id' => 'orderline-item_id', 'name' => 'OrderLine[item_id]'])
	...
	<?= Form::widget([
		    ...
		    'attributes' => [
		        'image_add' => [
					...
					'options' => ['inline'=>true],
					'items' => [
						OrderLine::IMAGE_ADD => Yii::t('store', 'Add'),
						OrderLine::IMAGE_REPLACE => Yii::t('store', 'Replace')
					]
				],
			],
		])
	...
	<?php
		...
		$detail = $model->getDetail();
	 	if($model->item_id == $chroma_item->id)
			echo $this->render('../order-line-detail/_update_chroma', [
			    'model' => $model,
			    'form' => $form,
				'detail' => $detail
		    ]);
		else if($model->item_id == $fineart_item->id)
			echo $this->render('../order-line-detail/_update_fineart', [
			    'model' => $model,
			    'form' => $form,
				'detail' => $detail
		    ]);
		else if($model->item_id == $free_item->id)
			echo $this->render('../order-line-detail/_update_free');
	?>
