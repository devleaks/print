...
class OrderController extends Controller
{
    ...
    public function actionSearch($search = null)
    {
		...
		if($search)
			$dataProvider->query
				->with('client')
				->orWhere(['like', 'order.name', $search])
				->orWhere(['like', 'order.reference', $search])
				->orWhere(['like', 'order.reference_client', $search])
    ...
    public function actionCreate($id = null, $type = Document::TYPE_ORDER)
    {
        ...
        if ($model->load(Yii::$app->request->post())) { // we just create the order
			if(!isset($model->order_type)) $model->order_type = $type;
			...
			if(!isset($model->name)) {
				switch($model->order_type) {
					...
					case Document::TYPE_BID:
						$o = Parameter::getTextValue('application', 'devis', '-YII-');
						$model->name = substr($model->due_date,0,4).$o.Sequence::nextval('bid_number');
						break;
				}
			}
			// temporaily set reference
			// $model->reference = $model->order_type.'::'.$model->name;
			//Yii::trace('order nr: '.$model->name);
			if ($model->save()) {
				$model->status = Document::STATUS_OPEN;
				$model->save();
				$addOrderLine = OrderLineController::addFirstLine($model);
				$model->updatePrice();
				return $this->redirect(['order-line/create', 'id' => $model->id]);
			}
        } else {
			if(!isset($model->order_type)) $model->order_type = $type;
    ...
    public function actionLiveUpdate($id)
    {
        $model = $this->findModel($id);
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
			Yii::$app->session->setFlash('info', Yii::t('store', '{document} updated', ['document' => Yii::t('store', $model->order_type)]).'.');
        }
	...
	public function actionConvert($id) {
		$model = $this->findModel($id);
		if($model->order_type == Document::TYPE_ORDER && $model->bom_bool) {
	...
	public function actionSubmit($id) {
		...
		if($work)
        	return $this->redirect(['/work/work/view', 'id' => $work->id]);
		else {
		...
		if($captureEmail->load(Yii::$app->request->post())) {
			...
			if($captureEmail->email != '') {
				...
				Yii::$app->mailer->compose()
				    ...
				    ->setSubject(Yii::t('store', $model->order_type).' '.$model->name)
