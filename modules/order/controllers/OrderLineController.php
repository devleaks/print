...
class OrderLineController extends Controller
{
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
			//Yii::trace('OrderLineController::actionUpdate: '.$model->image_add);
			if($model->image_add == OrderLine::IMAGE_REPLACE)
			...
			$newOrderLine->order_id = $model->order_id;
            return $this->redirect(Url::to(['create', 'id' => $model->order_id]));
        } else {
    ...
    public function actionCreate($id)
    {
        ...
        if ($order->load(Yii::$app->request->post()) && $order->save()) {
			Yii::$app->session->setFlash('info', Yii::t('store', '{document} updated', ['document' => Yii::t('store', $order->order_type)]).'.');
		}
	...
	protected static function updateDetail($model, $detail) {
		if($_POST){
			//Yii::trace('OrderLineController::createDetail: '.$model->id);
			$detail->order_line_id = $model->id;
			if ($detail->load(Yii::$app->request->post())) {
				//Yii::trace('OrderLineController::createDetail: 2: '.$detail->order_line_id);
				...
				if($model->item_id == $free_item->id) { // copy temporary label to note, do not save "details"
				 	...
				 	if($detail->save()) {
						//Yii::trace('OrderLineController::createDetail: 3: '.$detail->id);
						return true;
					}
				}
			}
	...
	protected static function createDetail($model) {
		...
		if(! in_array($model->item_id, $special_items) ) {
			//Yii::trace('OrderLineController::createDetail: 1: '.$model->item_id.' not in '.print_r($special_items, true).'.');
			return false;
		}
	...
	protected static function loadImages($model) {
		if($_POST){
		    ...
		    foreach($uploadedFiles as $idx => $image) {
		        ...
		        $picture->order_line_id = $model->id;
