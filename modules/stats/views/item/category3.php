...
foreach($dataProvider->query->each() as $m) {
	if($m['category'] == '') $m['category'] = 'Sans catégorie';
	$groups[] = [$m['category'], intval($m['tot_count'])];
}

$groups2 = [];
foreach($dataProvider2->query->each() as $m) {
	if($m['category'] == '') $m['category'] = 'Sans catégorie';
	$groups2[] = [$m['category'], intval($m['tot_count'])];
}

$this->title = Yii::t('store', 'Items').' — Catégorie';
