...
if(isSet($opening_balance)) {
	...
	$openingBalance = [[
		'columns' => [
			['content'=> 'Opening Balance', 'options'=>['colspan'=>4, 'class'=>'text-right '.$color]],
...
if(isSet($closing_balance)) {
	...
	$closingBalance = [[
		'columns' => [
			['content'=> 'Closing Balance', 'options'=>['colspan'=>4, 'class'=>'text-right '.$color]],
...
//		'showPageSummary' => true,
        'columns' => [
	        ...
	        [
	            ...
	            'value' => function ($model, $key, $index, $widget) {
                    return $model->document ? $model->document->name . ($model->note != '' ? ' ('.$model->note.')' : ''): $model->note;
	            },
	            'format' => 'raw',
			],
//            'note',
			...
			[
				...
				'noWrap' => true,
//				'pageSummary' => function ($summary, $data, $widget) { return 'Count is ' . print_r($data[count($data)-1], true); }
			],
        ],
    ]); ?>
